...
int main() {
	...
	try {
		...
		renderer.materialMap.Init(16, (1 << 18));
...

		{
			...
			std::ifstream ifile("world_larger.txt");
			if (!ifile.good()) {
				...
				for (UINT y = 0; y < sizeY; y++) {
					for (UINT z = 0; z < sizeZ; z++) {
						...
						if (blockMap.contains(blockIndex)) {
							renderer.materialMap.SetIndex({ x * 16, y * 16, z * 16 }, blockMap.at(blockIndex), renderer.materialMap.numLayers - 1 - 4);
							continue;
						}
						...
						for (UINT x2 = 0; x2 < 16; x2++) {
							for (UINT y2 = 0; y2 < 16; y2++) {
								for (UINT z2 = 0; z2 < 16; z2++) {
									UINT col;
									if (rand() % 2)
										col = *(UINT*)img.GetPixel(z2, x2);
									else
										col = *(UINT*)img.GetPixel(y2, x2);
									if (((byte*)&col)[3] == 0)
										continue;
									if (!matList.contains(col)) {
										std::cout << "Matlist didn't contain " << ((Peio::Byte4*)&col)->ToString() << std::endl;
										exit(0);
									}
									renderer.materialMap.SetIndex({ x * 16 + x2, y * 16 + y2, z * 16 + z2 },
										matList.at(col));
								}
							}
						}
						...
						//	}
						//}
						blockMap.insert({ blockIndex, renderer.materialMap.GetIndex({ x * 16, y * 16, z * 16 }, renderer.materialMap.numLayers - 1 - 4) });
					}
				}
			}
			ifile.close();
		...
		Peio::FunctionHandler<Peio::Win::RawMouseButtonDownEvent> mouseButtonDownHandler(
			[&renderer, &graphics](Peio::Win::RawMouseButtonDownEvent* event) -> bool {
				if (event->button == Peio::Win::MouseButton::RIGHT && event->foreground) {
					...
					//renderer.materialMap.SetIndex(result.voxel, Peio::Vxl::MaterialMap::null);
