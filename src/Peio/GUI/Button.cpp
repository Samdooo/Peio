...
namespace Peio::GUI {
	...
	void Button::Handle(Win::MouseMoveEvent& event)
	{
	...
	void Button::Handle(Win::MouseButtonDownEvent& event)
	{
		bool hovered = IsHovered(event.position);
		if (hovered) {
			OnMouseDown(event);
	...
	void Button::Handle(Win::MouseButtonUpEvent& event)
	{
		bool hovered = IsHovered(event.position);
		if (hovered) {
			OnMouseUp(event);
	...
	void Button::Handle(Win::MouseWheelEvent& event)
	{
		bool hovered = IsHovered(event.position);
		if (hovered) {
			OnMouseWheel(event);
